...
const ImageWithLabel = {
    view: vnode => mithril("figure", [
        mithril('img.hero', {src: vnode.attrs.src}),
        mithril("span.label", mithril.trust(vnode.attrs.label))
    ])
};
...
const MasonryTile = {
    ...
    view: vnode => mithril("div.tile", mithril("figure", [
        mithril('img', {src: vnode.attrs.src, onclick: () => { mithril.route.set("/image/" + vnode.attrs.checksum);}}),
        mithril("p", mithril.trust(vnode.attrs.label))
    ]))
};

const Hero = {
  view: vnode => mithril(".hero", [
    mithril(ImageWithLabel, {src: "resources/MWS00SelfGod.jpg", label: "Self God"})
  ])
};

const Logo = {
  view: vnode => mithril('img.hamsa-logo[src=images/hamsa-logo.jpg]', {click: ev => mithril.route.set("/")})
};

const MonasteryLogo = {
  view: vnode => mithril('img.monastery-logo[src=images/monastery-logo.png]')
};

const Header = {
  view: vnode => {
    return mithril("header", [
      mithril(Logo),
      mithril('div', {style:"flex: 1 1 auto"}, [
        mithril('h1.title', 'Himalayan Academy'),
        mithril('h2.subtitle', 'Art & Photo Collection')
      ]),
      mithril(MonasteryLogo)
    ])
  }
};
...
const Masonry = {
    ...
    view: vnode => mithril("div.collection", [
        mithril("h1", "Collection"),
        mithril("section.masonry", vnode.state.images.length > 0 ? vnode.state.images.map(image => {
                console.log("thumb", image.thumbnail);
                var t = "http://localhost:8080" + image.thumbnail;
                return mithril(MasonryTile, {src: t, label: "Short description", checksum: image.checksum})
            })
            :
            mithril("h1", "Loading")
        )
...
const CollectionLayout = {
    ...
    view: vnode => [
        mithril(Header),
        mithril(Hero),
        mithril(Masonry, {images: vnode.state.images})
    ]
};
...
const SingleItemLayout = {
    image: false,
    oninit: vnode => {
        checksum = mithril.route.param("checksum");
    ...
    view: vnode => [
        mithril(Header),
        mithril("figure", [
            mithril("img.is-single-item-display", {src: "http://localhost:8080" + vnode.state.image.path }),
            mithril("p", mithril.trust(vnode.attrs.label))
        ])
